...
function Account(){
    ...
    async function modificaDati(){
        ...
        let response = await fetch("https://alpakas.herokuapp.com/modificaaccountd",{
            method: "post",
            headers: {
    ...
    const valida = () => {
        ...
        const stampadati = () => {
            ...
            if(profile.numeroIdentificativoPatente === ""){
                dati.innerHTML += "<div class='container'>" +
                "<p>Nome: " + profile.nome + "</p>" +
                "<p>Cognome: " + profile.cognome + "</p>" +
                "<p>Email: " + profile.email + "</p>" +
                "<p>Cellulare: " + profile.numeroDiTelefono + "</p>" +
                "<p>Numero Patente: Nessuna patente inserita </p>" +
                "<p>Data di Nascita: " + profile.dataDiNascita + "</p>" +
                "<p>Luogo di Nascita: " + profile.luogoDiNascita + "</p>" +
                "</div><br></br>"
            } else {
                dati.innerHTML += "<div class='container'>" +
                "<p>Nome: " + profile.nome + "</p>" +
                ...
                "<p>Email: " + profile.email + "</p>" +
                "<p>Cellulare: " + profile.numeroDiTelefono + "</p>" +
                "<p>Numero Patente: " + profile.numeroIdentificativoPatente + "</p>" +
                "<p>Categoria Patente: " + profile.tipologiaPatente + "</p>" +
                "<p>Data di Nascita: " + profile.dataDiNascita + "</p>" +
                "<p>Luogo di Nascita: " + profile.luogoDiNascita + "</p>" +
                "</div><br></br>"
            }
    ...
    return (
        ...
        <div class="form-group">
            ...
            <input type="text" name="tipologiaPatente" class="form-control"  placeholder="A-B-C-D-" onChange={(event) => { setTipologiaPatente(event.target.value);}}/>
            <br></br>
        </div>
        <div class="form-group">
            <label >Cellulare</label>
            <input type="email" name="cellulare" class="form-control" placeholder="(+39) XXXXXXXXXX" onChange={(event) => { setNumeroDiTelefono(event.target.value);}}/>
            <br></br>
        </div>
        <div class="form-group">
            <label>Password</label>
            <input type="password" name="password" class="form-control" id="exampleInputPassword1" placeholder="Password" onChange={(event) => { setPassword(event.target.value);}}/>
            <br></br>
        </div>
        <div class="form-group">
            <label>Conferma Password</label>
            <input type="password" name="confermaPassword" class="form-control" id="exampleInputPassword1" onChange={(event) => { setconfermaPassword(event.target.value);}} placeholder="Password"/>
            <br></br>
        </div>
